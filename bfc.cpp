...
I8 bfc::interp(FILE *f) {
	I8 *rd_buf  = new I8[nasm_rdbuf]();
	I8 *cellbuf = new I8[ccount * csize]();
	I8 *cellptr = cellbuf;
	U64 cell_idx = 0;
	auto interp_match_loop = [&](insn& ins) {
		for (U64 i = 0; i < insns.size(); i++) {
			if (ins.type == insn_e::LST) {
				if (insns[i].type == insn_e::LEN && ins.arg == insns[i].arg)
					return i;
			} else { // INSN_LEN
				if (insns[i].type == insn_e::LST && ins.arg == insns[i].arg)
					return i;
			}
		}
		return insns.size();
	...
	#define DR8  (*cellptr)
	#define DR16 (*((I16 *) cellptr))
	#define DR32 (*((I32 *) cellptr))
	#define DR64 (*((I64 *) cellptr))
	#define PRINT_CELL() {\
		switch(csize) {\
			case 1: printf("%hhd\n", DR8);  break;\
			case 2: printf("%hd\n",  DR16); break;\
	...
	auto interp_inc = [&](insn& ins) {
		DEBUG printf("cell @ %lu: ", cell_idx);
		switch(csize) {
			case 1: DR8  += ins.arg; DEBUG printf("%hhd\n", DR8);  break;
			case 2: DR16 += ins.arg; DEBUG printf("%hd\n",  DR16); break;
			case 4: DR32 += ins.arg; DEBUG printf("%d\n",   DR32); break;
			case 8: DR64 += ins.arg; DEBUG printf("%ld\n",  DR64); break;
		}
	};
	auto interp_dec  = [&](insn& ins) {
		DEBUG printf("cell @ %lu: ", cell_idx);
		switch(csize) {
			case 1: DR8  -= ins.arg; DEBUG printf("%hhd\n", DR8);  break;
			case 2: DR16 -= ins.arg; DEBUG printf("%hd\n",  DR16); break;
	...
	auto interp_skip_loop = [&](insn& ins, U64& insn_idx) {
		// return = continue to code after LST
		switch(csize) {
			case 1: if (DR8  != 0) { DEBUG putchar('\n'); return; }; break;
			case 2: if (DR16 != 0) { DEBUG putchar('\n'); return; }; break;
			case 4: if (DR32 != 0) { DEBUG putchar('\n'); return; }; break;
			case 8: if (DR64 != 0) { DEBUG putchar('\n'); return; }; break;
		}
		DEBUG printf("skip\n");
		insn_idx = interp_match_loop(ins);
	};
	auto interp_loop = [&](insn& ins, U64& insn_idx) {
		switch(csize) {
			case 1: if (DR8  == 0) { DEBUG putchar('\n'); return; }; break;
			case 2: if (DR16 == 0) { DEBUG putchar('\n'); return; }; break;
			case 4: if (DR32 == 0) { DEBUG putchar('\n'); return; }; break;
		...
		insn_idx = interp_match_loop(ins);
		DEBUG printf("loop -> %lu\n", insn_idx);
	};
	for (U64 insn_idx = 0; insn_idx < insns.size(); insn_idx++) {
		insn ins = insns[insn_idx];
		DEBUG printf("DEBUG: ins @ %lu [%s %lu];\t\t", insn_idx, get_insn_name(ins.type).c_str(), ins.arg);
		switch(ins.type) {
			case insn_e::INC: interp_inc(ins); break;
			case insn_e::DEC: interp_dec(ins); break;
			case insn_e::PTI:
				if (cell_idx + ins.arg >= ccount) {
					error.location = insn_idx + 1;
					error.where    = "bfc::interp()";
					error.desc     = '[';
					error.desc    += get_insn_name(ins.type);
					error.desc    += ' ';
					error.desc    += std::to_string(ins.arg);
					error.desc    += "] Pointer increment out of bounds";
					goto interp_error;
				}
				cellptr  += ins.arg * csize;
				cell_idx += ins.arg;
				DEBUG {
					printf("cell @ %lu: ", cell_idx);
					PRINT_CELL();
				}
				break;
			case insn_e::PTD:
				if ((I64) (cell_idx - ins.arg) < 0) {
					error.location = insn_idx + 1;
					error.where    = "bfc::interp()";
					error.desc     = '[';
					error.desc    += get_insn_name(ins.type);
					error.desc    += ' ';
					error.desc    += std::to_string(ins.arg);
					error.desc    += "] Pointer decrement out of bounds";
					goto interp_error;
				}
				cellptr  -= ins.arg * csize;
				cell_idx -= ins.arg;
				...
				break;
			case insn_e::OUT:
				for (U64 i = 0; i < ins.arg; i++)
					putchar(*cellptr);
				DEBUG putchar('\n');
				break;
			case insn_e::IN:
				if (csize == 1) {
					::read(0, cellptr, ins.arg + 1);
					break;
				}
				memset(rd_buf, 0, nasm_rdbuf);
				::read(0, rd_buf, ins.arg + 1);
				for (U64 i = 0; i < strlen(rd_buf); i++) {
					*(cellptr + i * csize) = rd_buf[i];
				}
				break;
			case insn_e::LST:
				interp_skip_loop(ins, insn_idx);
				break;
			case insn_e::LEN:
				interp_loop(ins, insn_idx);
				break;
		}
	}
	delete[] rd_buf;
